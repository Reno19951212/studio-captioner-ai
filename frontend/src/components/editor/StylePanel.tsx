...
const DEFAULT_STYLE: SubtitleStyleConfig = {
  fontFamily: "Noto Sans SC", fontSize: 24, fontColor: "#ffffff",
  outlineColor: "#000000", outlineWidth: 2, bgEnabled: false,
  bgColor: "#000000", bgOpacity: 0.75, position: "bottom",
};

interface StylePanelProps { onChange?: (style: SubtitleStyleConfig) => void; }

export function StylePanel({ onChange }: StylePanelProps) {
  const [style, setStyle] = useState<SubtitleStyleConfig>(DEFAULT_STYLE);
  const update = (patch: Partial<SubtitleStyleConfig>) => { const next = { ...style, ...patch }; setStyle(next); onChange?.(next); };
  ...
  return (
    <div className="space-y-3 p-3 text-sm">
      ...
      <div>
        ...
        <input type="range" min={12} max={72} value={style.fontSize} onChange={(e) => update({ fontSize: Number(e.target.value) })} className="w-full" />
      </div>
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="block text-xs text-zinc-500 mb-1">Font Color</label>
          <input type="color" value={style.fontColor} onChange={(e) => update({ fontColor: e.target.value })} className="w-full h-8 bg-zinc-800 border border-zinc-700 rounded cursor-pointer" />
        </div>
        <div>
          <label className="block text-xs text-zinc-500 mb-1">Outline Color</label>
          <input type="color" value={style.outlineColor} onChange={(e) => update({ outlineColor: e.target.value })} className="w-full h-8 bg-zinc-800 border border-zinc-700 rounded cursor-pointer" />
        </div>
      </div>
      <div>
        <label className="block text-xs text-zinc-500 mb-1">Outline Width: {style.outlineWidth}px</label>
        <input type="range" min={0} max={6} value={style.outlineWidth} onChange={(e) => update({ outlineWidth: Number(e.target.value) })} className="w-full" />
      </div>
      <div>
        ...
        {style.bgEnabled && (
          <div className="mt-2 grid grid-cols-2 gap-2">
            <div>
              ...
              <input type="color" value={style.bgColor} onChange={(e) => update({ bgColor: e.target.value })} className="w-full h-8 bg-zinc-800 border border-zinc-700 rounded cursor-pointer" />
            </div>
            <div>
              <label className="block text-xs text-zinc-500 mb-1">Opacity: {Math.round(style.bgOpacity * 100)}%</label>
              <input type="range" min={0} max={100} value={style.bgOpacity * 100} onChange={(e) => update({ bgOpacity: Number(e.target.value) / 100 })} className="w-full" />
            </div>
          </div>
        )}
      </div>
